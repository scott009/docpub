...
async function generatePdf() {
    try {
        // Read input files
        const jsonPath = path.join(__dirname, 'reOrdInq12.json');
        const cssPath = path.join(__dirname, 'ada2.css');
                ...
                switch(item.type) {
                    ...
                    case 'ol':
                        const items = (item.items || []).map(li => 
                            `<li${li.class ? ` class="${li.class}"` : ''}>${li.text || ''}</li>`
                        ).join('');
        ...
        // Generate PDF
        const pdfPath = path.join(__dirname, 'reOrdInq12.pdf');
        await page.pdf({
            path: pdfPath,
            format: 'A4',
